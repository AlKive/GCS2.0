...
const SdCardIcon = () => <svg className="w-5 h-5 mr-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="7" y="2" width="10" height="20" rx="2" ry="2"/><path d="M7 6h10M7 10h10M9 14h6M9 18h6"/></svg>;

export type View = 'dashboard' | 'analytics' | 'flightLogs' | 'settings' | 'guide' | 'about' | 'droneStream' | 'offlineManager';
  ...
  return (
    <aside className="w-72 bg-black/40 backdrop-blur-xl flex flex-col hidden lg:flex border-r border-white/5 z-20 shadow-2xl transition-colors duration-300">
      ...
      <nav className="flex-1 pt-6 flex flex-col">
        ...
        <NavItem icon={<SdCardIcon />} label="OFFLINE MANAGER" active={currentView === 'offlineManager'} onClick={() => onNavigate('offlineManager')} />
